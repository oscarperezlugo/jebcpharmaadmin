<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="VentasVend.aspx.cs" Inherits="PanelAdmin.VentasVend" %>
...
<PagerStyle CssClass="pgr"></PagerStyle>
                    ...
                    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:PaladarMobileConnectionString %>" SelectCommand="SELECT iDCliente, FechaVenta, Monto, Lineas, SubTotal, iDVenta, Row, Status, TipoVenta, Metodo, Direccion, Rif, Tipo FROM Cabecera WHERE (MONTH(FechaVenta) = @Mes) AND (YEAR(FechaVenta) = YEAR(GETDATE())) AND Vendedor = @Vendedor">
                    <SelectParameters>
            <asp:CookieParameter CookieName="mesR" Name="Mes" Type="String" />
            <asp:CookieParameter CookieName="vendedorR" Name="Vendedor" Type="String" />
        </SelectParameters>
                    </asp:SqlDataSource>
                    </div>
            </div>
